...
class HaikuChecker(
    ...
    private val systemPrompt = """
        ...
        родной язык ученика. «Kako si?» — это и «как дела?», и «как ты?», и
        «как поживаешь?»; все три верны.

        Пунктуация и заглавные буквы не проверяются вовсе. Пропущенная точка,
        запятая, восклицательный или вопросительный знак, строчная буква в
        начале — не ошибка и не повод для correct = false. Ответ набирают на
        телефоне, и знаки там стоят денег; проверяется знание языка, а не
        аккуратность набора.
