...
interface AppDao {
    ...
    /**
     * Убрать карточки выброшенных слов (`Excluded`).
     *
     * Единственное место в приложении, где прогресс удаляется. Оно нужно
     * потому, что слово исчезает из словаря, а карточка остаётся: заданий по
     * ней больше не построить, но `vocabDue` считает по таблице и держал бы
     * её в «просрочено» до скончания века — счётчик, который нельзя разобрать
     * ничем.
     *
     * Список приходит готовым, а не собирается запросом по образцу: разбирает
     * идентификатор `Vocab.lemmaOf`, и вторая, самодельная разборка в SQL
     * однажды разошлась бы с ней.
     */
    @Query("DELETE FROM cards WHERE exerciseId IN (:ids)")
