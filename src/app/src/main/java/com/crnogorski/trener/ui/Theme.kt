 ...
 *
 * Тёмная тема — золото по чернилам, светлая — тёмно-оранжевый по льду.
...
/**
 * Светлая тема — холодный фон, белые карточки, тёплый акцент.
 *
 * Акцент здесь тёмно-оранжевый, а в тёмной теме золотой, и это не оплошность:
 * `accent` — роль, а не оттенок. Ровно поэтому цвет и назван по роли: `Accent`,
 * а не `Gold`, — читающий код не должен гадать, какого он цвета в этой теме.
 *
 * Оранжевый пришёл на смену синему по жалобе владельца: синий по голубоватому
 * фону читался холодно и сливался с ним. Заодно починился контраст — прежний
 * `0xFF1F6FA8` давал на `surface2` всего 4,13 к 1, то есть ниже порога, а
 * нынешний даёт 4,66. По белой карточке — 6,07, по фону — 5,43.
 *
 * Акцент, зелень и красный взяты заметно темнее, чем в тёмной теме: на светлом
 * фоне исходные оттенки не читались бы ни текстом, ни рамкой. Оттенок выбран
 * так, чтобы работать и текстом по фону, и подложкой кнопки со светлым текстом.
 */
