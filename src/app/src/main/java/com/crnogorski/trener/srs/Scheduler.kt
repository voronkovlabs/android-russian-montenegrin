...
object Scheduler {
     ...
     */
    /**
     * Отложено ли слово рукой человека.
     *
     * Отдельного поля под это нет: «Отложить на потом» просто сдвигает
     * [CardEntity.dueAt] на две недели, не трогая интервал. Отсюда и признак —
     * **срок дальше, чем дал бы собственный интервал карточки**. Обычная
     * карточка после ответа стоит ровно на свой интервал вперёд и с каждым
     * часом приближается, а отложенная выпрыгивает за него.
     *
     * Сравнивать с «созрело ли» (`dueAt <= now`) тут нельзя: тренировка вне
     * расписания по замыслу берёт и несозревшее, и такой фильтр убил бы её
     * целиком. Нужен ровно тот случай, когда человек сказал «мне это сейчас
     * не нужно».
     */
    fun snoozed(card: CardEntity, now: Long): Boolean =
