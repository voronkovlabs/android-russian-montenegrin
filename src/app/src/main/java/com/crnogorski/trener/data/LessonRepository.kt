...
object LocalCheck {
    ...
    /**
     * Какие слова эталона движок действительно расслышал.
     *
     * Возвращает номера слов **в показанном тексте**, а не в свёрнутом: их
     * подсвечивает `GlossedText`, и считать он должен то же, что видит человек.
     * Поэтому слова берутся тем же выражением, что и подсказки по словам, а
     * свёртка применяется к каждому по отдельности.
     *
     * Заведено по просьбе владельца: «показывать в самом читаемом тексте
     * правильно распознанные слова зелёным». Число «совпало 5 из 7» говорит,
     * сколько, но не говорит, **какие**, — а перечитывать вслепую бесполезно,
     * потому что не видно, что именно не дошло.
     *
     * Совпадение то же, что в счёте ([close]): знак-другой прощается.
     *
     * Оговорка, которую надо знать: числительные здесь **не** собираются в
     * число, как в [words] («dvadeset dva» → «22»). В тексте с цифрами счёт и
     * подсветка могут разойтись на слово. В историях цифр нет вовсе, а
     * городить второй разбор ради этого дороже самой беды.
     */
    fun matchedWords(heard: String, expected: String): Set<Int> {
        // Показанное слово не всегда одно слово для сверки: `tamo-amo` движок
        // слышит двумя, и [normalize] делит его по дефису так же. Поэтому
        // сверяем по частям, помня, чьи они, а зелёным показываем слово
        // целиком и только когда расслышаны **все** его части: половина
        // подсвеченного слова говорила бы неправду.
        val shown = mutableListOf<String>()
        val owner = mutableListOf<Int>()
        val parts = mutableListOf<Int>()
        SHOWN_WORD.findAll(expected).forEach { match ->
            val pieces = reflex(flatten(match.value)).split(' ').filter { it.isNotBlank() }
            parts += pieces.size
            pieces.forEach {
                shown += CANON[it] ?: it
                owner += parts.size - 1
        ...
        val got = words(heard)
        if (shown.isEmpty() || got.isEmpty()) return emptySet()

        val dp = Array(shown.size + 1) { IntArray(got.size + 1) }
        for (i in shown.indices) {
            for (j in got.indices) {
                dp[i + 1][j + 1] = if (close(shown[i], got[j])) {
                    dp[i][j] + 1
                } else {
                    maxOf(dp[i][j + 1], dp[i + 1][j])
                }
        ...
        // Обратный ход по той же таблице: сама длина ничего не говорит о том,
        // какие слова в неё вошли.
        val hits = IntArray(parts.size)
        var i = shown.size
        var j = got.size
        while (i > 0 && j > 0) {
            when {
                close(shown[i - 1], got[j - 1]) && dp[i][j] == dp[i - 1][j - 1] + 1 -> {
                    hits[owner[i - 1]]++
                    i--
                    j--
                }
        ...
        return parts.indices.filter { hits[it] == parts[it] && parts[it] > 0 }.toSet()
    }

    /**
      * Слово показанного текста.
      *
      * Публичное и одно на всех намеренно: по нему `GlossedText` расставляет
      * подсказки и зелень, [matchedWords] считает номера, а прогон корпуса
      * перечисляет потерянные слова. Три копии одного выражения однажды
      * разошлись бы, и номера слов перестали бы сходиться — а заметить это
      * можно было бы только по неверно подсвеченному слову.
      */
    val SHOWN_WORD = Regex("[\\p{L}-]+")

    /**
     * Текст на предложения — по одному на заход распознавания.
     *
     * Движок закрывает заход **по паузе**, а не по концу фразы, и просьбы не
     * обрывать запись документация разрешает игнорировать. Значит граница
     * предложения и есть граница захода: длинный текст читается по частям
     * (ReadingAnswer), а истории режутся по предложениям заранее.
     *
     * Правило живёт здесь, а не на экране, потому что оно про текст: по нему
     * же прогон корпуса решает, сколько заходов нужно заданию на чтение, и
     * второй такой же регуляркой эти два места однажды разошлись бы.
     */
    fun sentences(text: String): List<String> =
        SENTENCE.findAll(text)
            .map { it.value.trim() }
            .filter { it.isNotBlank() }
            .toList()
    ...
    fun readingScore(heard: String, expected: String): ReadingScore {
        ...
        for (i in want.indices) {
            for (j in got.indices) {
                // Скидка по знакам — та же, что у речи ([close]). До 2.1 здесь
                // стояло **точное** совпадение слова, и это было не решением, а
                // недосмотром: скидка появилась в 1.80 вместе с `spokenScore`, а
                // сюда её не перенесли.
                //
                // Расхождение работало в худшую сторону: чтение вслух длиннее и
                // труднее речи по фразе, а спрашивалось строже. На живом чтении
                // сказок (20.09.2026) `bumbašira` против услышанного
                // `bumbašara` и `rasrdi` против `rasarde` роняли отрезок ниже
                // порога — один знак разницы, а предложение непроходимо.
                // По двум снимкам владельца: 4 из 8 против 6 из 8 и 5 из 7
                // против 6 из 7, то есть провал против зачёта.
                dp[i + 1][j + 1] = if (close(want[i], got[j])) {
                    dp[i][j] + 1
                } else {
                    maxOf(dp[i][j + 1], dp[i + 1][j])
