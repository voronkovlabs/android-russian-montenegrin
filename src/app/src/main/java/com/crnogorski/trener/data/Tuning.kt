...
data class Tuning(
    ...
    data class Srs(
        ...
        /**
         * Через сколько дней вернуть **словарную** карточку после ошибки.
         *
         * У слов отдельное правило, и это не прихоть: десятиминутный возврат
         * хорош, когда материала мало и промах видно тут же. В словаре из
         * трёхсот слов он превращает занятие в толчение одного и того же —
         * ровно на это владелец и пожаловался. Слово, в котором ошиблись,
         * возвращается **завтра**, а не через десять минут.
         *
         * Плата прямая: ступень переучивания пропадает. Промахнулся — увидишь
         * не сразу, а через день, и до тех пор ошибка ничем не поправлена.
         */
        val vocabLapseDays: Int = 1,
        /** Через сколько часов вернуть пропущенное задание. */
        val skipHours: Int = 4,
        /** Сколько карточек берётся в один подход повторения. */
        val reviewLimit: Int = 25,
        /**
         * На сколько дней прячется слово, отложенное кнопкой «потом».
         *
         * Две недели — «ощутимо», как просил владелец: слово должно уйти из
         * глаз, а не мелькнуть через день. Отложить — не ошибка и не пропуск:
         * это «сейчас мне это не нужно», и наказывать за такое нечем.
         */
        val snoozeDays: Int = 14,
        /**
    ...
    data class Daily(
        ...
        /**
         * Сколько тишины прощается, прежде чем время перестаёт засчитываться.
         *
         * По жалобам 116 и 121. Это не про жульничество, а про честность
         * «секунд на ответ» в отчёте: отвлечение, посчитанное занятием, портит
         * единственное число, которое мерит беглость.
         *
         * Десять секунд — столько молча думают над переводом. Дольше отличить
         * раздумье от телефона на столе нечем. Ноль возвращает прежнее
         * поведение: время идёт всегда, до потолка в полторы минуты.
         */
        val idleSeconds: Int = 10
    )
    ...
    data class Vocab(
        /**
         * Сколько слов держим в обороте одновременно.
         *
         * Новые слова заводятся, пока незаученных меньше этого числа, и
         * перестают, когда пул набран. Отсюда и берётся «каждый раз новый
         * набор»: при трёхстах словах в обороте и двадцати пяти карточках за
         * заход одно и то же слово попадается примерно раз в дюжину заходов, а
         * не через раз.
         *
         * Это **замена дневной норме** [newPerDay], а не добавка к ней: норма
         * ограничивала, сколько новых слов можно взять за день, и при большом
         * пуле мешала бы его набрать. Норма осталась работать в ежедневном
         * задании, где счёт идёт на минуты, а не на слова.
         *
         * Цена названа честно: чем больше пул, тем реже встречается каждое
         * слово и тем дольше идти до десяти встреч, после которых слово
         * считается выученным. Это обмен разнообразия на скорость, а не
         * бесплатное улучшение. Ноль возвращает прежнее поведение — новые
         * слова только по дневной норме.
         */
    ...
    fun sane(): Tuning = Tuning(
        srs = srs.copy(
            ...
            vocabLapseDays = srs.vocabLapseDays.coerceIn(0, 30),
            skipHours = srs.skipHours.coerceIn(1, 72),
            reviewLimit = srs.reviewLimit.coerceIn(1, 200),
            snoozeDays = srs.snoozeDays.coerceIn(1, 365),
        ...
        daily = daily.copy(
            ...
            // Ноль — выключено. Потолок в минуту: дольше всё равно срежет
            // `Pace.SAMPLE_MAX`, и порог перестал бы что-либо значить.
            idleSeconds = daily.idleSeconds.coerceIn(0, 60)
        ),
        vocab = vocab.copy(
            poolTarget = vocab.poolTarget.coerceIn(0, 2000),
