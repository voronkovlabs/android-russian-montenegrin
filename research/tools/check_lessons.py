...
**Замечания нельзя применять не проверив, и это не осторожность, а опыт.**
23.09.2026 прогон по t01 и t02 дал три замечания, и **все три оказались
неверны**:

* `smokava` объявлено именительным множественного (это родительный
  множественного, `smokve` — именительный), а предложенная замена `smokvi`
  вообще дательный единственного. Применить её значило бы **сломать верное
  задание**;
* `prođe` названо прошедшим временем. Это настоящее время совершенного
  глагола; прошедшее было бы `prošla je`;
* в ответе `word_bank` потребована точка, хотя ответ там собирается из плашек
  и знаков препинания не содержит нигде в курсе.

Проверять надо **по данным, а не по мнению**: парадигмы лежат в
`assets/vocab/forms-*.json` (из srLex), и спор о падеже решается за минуту
поиском леммы. Механизм иекавицы в приложении делает модель охочей до ярлыка
«это сербское» — та же болезнь, что описана в CLAUDE.md про `doktor`.

Польза от прогона при этом есть, просто не та: спор об объяснении к `t01e06`
вскрыл, что **наше** объяснение было неточным, хотя претензия модели была
неверной.
