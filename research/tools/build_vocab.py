...
GLOSS_FIX = {
    # Катя, жалобы 78 и 94: `morati` это глагол, а «должен» — краткое
    # прилагательное. Переводить часть речи другой частью речи нельзя.
    'morati': 'быть должным, быть обязанным',
    # Катя, жалоба 93: у `kao` и `kako` было одно толкование «как» на двоих.
    # Мало того что непонятно, какое «как» спрашивают, — слова с одинаковым
    # толкованием засчитываются наравне, и `kako` проходил вместо `kao`.
    'kao': 'как (при сравнении), словно',
    'kako': 'как (каким образом)',
    'vrata': 'дверь',
    'ključ': 'ключ',
    'krov': 'крыша',
    'kola': 'машина, автомобиль',
    'posao': 'работа, дело',
    # Разбор верха словаря по жалобе 79. Толкования пришли из источника не от
    # того слова: `lek` был «браком», `para` — «парой» при собственном примере
    # «Potpuno sam bez para» («совсем без денег»), `zvati` — только «звонить»
    # при примере «Kako se zove?».
    'lek': 'лекарство',
    'para': 'деньги (разг.); пар',
    'zvati': 'звать, называть; звонить',
}
