...
# Леммы, которые srLex даёт по хорватской норме. Курс черногорский, и
# показывать хорватский именительный падеж нельзя: `stol` это Загреб, а не
# Подгорица, где говорят `sto` (родительный у обоих `stola`, так что парадигма
# та же). Список ручной и должен таким остаться — выводить это правилом
# опасно, «-l» на конце бывает и законным.
LEMMA_FIX = {
    'stol': 'sto',
}
...
def read_lexicon(path, wanted_forms):
    ...
    **Но частота выбирает между чтениями только внутри одного разряда.**
    Служебное чтение почти всегда частотнее знаменательного — предлог `pod`
    встречается на порядок чаще существительного «пол», а числительное `sto`
    чаще стола, — и по голой частоте оба слова пропадали из курса вовсе:
    отбор видел NUM и ADP и выбрасывал их как служебные. Поэтому знаменательное
    чтение (существительное, глагол, прилагательное, наречие) имеет приоритет
    над служебным, а частота решает уже внутри знаменательных.

    Цена решения честная и её надо знать: в словарь приходят омографы, у
    которых знаменательное значение реже служебного. `sto` это и «сто», и
    «стол», и карточка научит только второму.
    """
    best = {}
    for line in gzip.open(path, 'rt', encoding='utf-8'):
        ...
        if form not in wanted_forms:
            continue
        freq, lemma, upos = int(p[6]), p[1].lower(), p[4]
        rank = (1 if upos in CONTENT else 0, freq)
        if form not in best or rank > best[form][0]:
            best[form] = (rank, lemma, upos)
    return {
        form: (LEMMA_FIX.get(lemma, lemma), upos)
        for form, (_, lemma, upos) in best.items()
    }
...
def read_paradigms(path, lemmas):
    ...
    [LEMMA_FIX] применяется и здесь, иначе исправление леммы отрезало бы её от
    собственных форм: `sto` искался бы в лексиконе как числительное «сто» и
    получил бы его парадигму, а формы стола (`stola`, `stolu`, `stolovi`)
    остались бы под хорватским `stol`, которого в словаре уже нет.
    """
    ...
    for line in gzip.open(path, 'rt', encoding='utf-8'):
        ...
        # У исправленной леммы чужая парадигма тоже есть (`sto` — числительное),
        # и смешивать их нельзя: берём только знаменательные разборы.
        if lemma in lemmas and (p[1].lower() not in LEMMA_FIX.values()
                                or p[4] in CONTENT):
...
def main(lexpath):
    ...
    for word, rank, count in gap:
        ...
        if upos not in CONTENT:
            ...
            continue
        # Два фильтра сняты 13.09.2026 по решению владельца, и оба считались
        # само собой разумеющимися.
        #
        # **«Курс уже знает»** выбрасывал слово, если оно встречалось в уроке,
        # истории или словаре подсказок. Посылка неверна: встретиться в
        # задании — не то же, что получить десять разнесённых повторений.
        # Именно этот фильтр держал за бортом `kuća`, `stan`, `soba`, `kola`,
        # и он же выкидывал тем больше, чем длиннее становился курс.
        #
        # **«Прозрачно для русского»** выбрасывал то, что русский узнаёт без
        # обучения. На длинных заимствованиях он прав (`balkon`, `muzika`), а
        # на коротких словах промахивается: `sto` отличается от «стол» одной
        # буквой и потому «прозрачен», хотя на слух это скорее «сто».
        # Владелец решил, что цена промаха выше цены лишнего слова.
        #
        # Оба остались **счётчиками**: по ним видно, сколько в списке такого,
        # что курс уже показывал и что похоже на русское, — это пригодится,
        # когда словарь придётся резать.
        if lemma in known_lemmas:
            dropped['курс уже знает (оставлено)'] += 1
        pos, text = gloss.get(lemma, ('', ''))
        if not text:
            dropped['нет перевода'] += 1
            continue
        if transparent(lemma, text):
            dropped['прозрачно для русского (оставлено)'] += 1
    ...
    # Тот же снятый фильтр, вид сбоку: слово выбрасывалось, если курс показал
    # хоть одну форму его парадигмы. Оставлен счётчиком по той же причине.
    dropped['курс знает форму (оставлено)'] = sum(
        1 for c in candidates
        if any(form in known for form, _, _, _ in paradigms.get(c[1], ()))
    )
